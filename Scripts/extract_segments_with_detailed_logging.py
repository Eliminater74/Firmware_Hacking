...
def extract_segments(firmware_file):
    ...
    for i, offset in enumerate(offsets):
        ...
        try:
            # Analyze the segment using 'file' command
            file_type = subprocess.check_output(['file', segment_file]).decode('utf-8')
            print(f"File type of {segment_file}: {file_type}")

            # Create a hexdump of the segment using 'xxd' command
            hexdump_file = os.path.join(segment_dir, f"{os.path.basename(segment_file)}.hexdump")
            with open(hexdump_file, 'wb') as hd:
                hexdump = subprocess.check_output(['xxd', segment_file])
                hd.write(hexdump)
                print(f"Hexdump saved to {hexdump_file}")

            data = open(segment_file, 'rb').read()
            decompressed = False
            ...
            if data.startswith(b'\x1F\x8B'):
                try:
                    ...
                    with open(os.path.join(segment_dir, f"{os.path.basename(segment_file)}.decompressed.gzip"), 'wb') as df:
                        df.write(decompressed_data)
                    print(f"Decompressed data saved to {os.path.basename(segment_file)}.decompressed.gzip")
                    decompressed = True
                except OSError as e:
                ...
                try:
                    ...
                    with open(os.path.join(segment_dir, f"{os.path.basename(segment_file)}.decompressed.bzip2"), 'wb') as df:
                        df.write(decompressed_data)
                    print(f"Decompressed data saved to {os.path.basename(segment_file)}.decompressed.bzip2")
                    decompressed = True
                except OSError as e:
                ...
                try:
                    ...
                    with open(os.path.join(segment_dir, f"{os.path.basename(segment_file)}.decompressed.xz"), 'wb') as df:
                        df.write(decompressed_data)
                    print(f"Decompressed data saved to {os.path.basename(segment_file)}.decompressed.xz")
                    decompressed = True
                ...
                try:
                    ...
                    with open(os.path.join(segment_dir, f"{os.path.basename(segment_file)}.decompressed.lz4"), 'wb') as df:
                        df.write(decompressed_data)
                    print(f"Decompressed data saved to {os.path.basename(segment_file)}.decompressed.lz4")
                    decompressed = True
                ...
                try:
                    ...
                    with open(os.path.join(segment_dir, f"{os.path.basename(segment_file)}.decompressed.zlib"), 'wb') as df:
                        df.write(decompressed_data)
                    print(f"Decompressed data saved to {os.path.basename(segment_file)}.decompressed.zlib")
                    decompressed = True
                except zlib.error as e:
                    print(f"Error decompressing {segment_file} with zlib: {e}")

            # Attempt to decompress with UPX
            upx_decompressed_file = os.path.join(segment_dir, f"{os.path.basename(segment_file)}.decompressed.upx")
            try:
                print(f"Attempting to decompress {segment_file} with UPX...")
                subprocess.run(['upx', '-d', segment_file, '-o', upx_decompressed_file], check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                print(f"Decompressed data saved to {upx_decompressed_file}")
                decompressed = True
            except subprocess.CalledProcessError as e:
                print(f"Error decompressing {segment_file} with UPX: {e}")
