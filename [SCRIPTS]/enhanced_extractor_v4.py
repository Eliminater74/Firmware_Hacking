...
def analyze_segment(segment_path):
    ...
    strings_command = f"strings {segment_path} | head -n 20"  # Limit the number of strings for readability
    entropy_command = f"ent {segment_path}"  # Entropy analysis

    file_output, _ = run_command(file_command)
    strings_output, _ = run_command(strings_command)
    entropy_output, _ = run_command(entropy_command)

    result['file'] = file_output.strip()
    result['strings'] = strings_output.strip()
    result['entropy'] = entropy_output.strip()
...
def try_extraction_tools(segment_path, output_dir):
    tools = [
        ...
        f"xz -d -c {segment_path} > {output_dir}/decompressed_segment",
        f"lzma -d -c {segment_path} > {output_dir}/decompressed_segment",
        f"bzip2 -d -c {segment_path} > {output_dir}/decompressed_segment"
    ]
                    ...
                    for extracted_file in extracted_files:
                        ...
                        if os.path.getsize(extracted_file_path) > 0:
                            ...
                            with open(result_file, 'w') as rf:
                                ...
                                rf.write("Entropy analysis:\n")
                                rf.write(result['entropy'])
