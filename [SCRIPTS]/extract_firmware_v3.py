...
def identify_and_extract(segment_file):
    ...
    if not found_compression:
        ...
        # Attempt manual inspection
        manual_inspection(segment_file)

def manual_inspection(segment_file):
    print(f"Performing manual inspection on {segment_file}...")

    # Hexdump
    hexdump_output, _ = run_command(f"hexdump -C {segment_file} | head -n 40")
    print(f"Hexdump of {segment_file}:\n{hexdump_output}")

    # Strings
    strings_output, _ = run_command(f"strings {segment_file} | head -n 40")
    print(f"Strings found in {segment_file}:\n{strings_output}")

    # Entropy Analysis
    entropy_output = calculate_entropy(segment_file)
    print(f"Entropy of {segment_file}: {entropy_output:.4f}")
